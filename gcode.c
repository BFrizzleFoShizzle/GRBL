...
uint8_t gc_execute_line(char *line) 
{
  ...
  while (line[char_counter] != 0) { // Loop until no more g-code words in line.
    ...
    if(letter == '*') {
      // checksum, not implemented, just eat it lol    
      float checksum;
      read_float(line, &char_counter, &checksum);
      continue;
    }
    ...
    switch(letter) {
      ...
      case 'M':
        ...
        switch(int_value) {
        	 ...
        	 break;
        	 // CBF implementing this, just pretend it works... :S
        	 case 110: 
				if(line[char_counter] == 'N') {
					char_counter++;
					float crap;
					// read some stuff in lol
					read_float(line, &char_counter, &crap);
				}
				else {
